...
def tmp_data_dir(tmp_path, monkeypatch):
    ...
    db_path.write_bytes(b"FAKE_SQLITE_CONTENT_FOR_TEST")
...
def test_create_backup_zip_contient_db_et_pdfs(tmp_data_dir):
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        assert zf.read("planning.db") == b"FAKE_SQLITE_CONTENT_FOR_TEST"
...
def test_restore_from_zip_round_trip(tmp_data_dir):
    ...
    # Pollue la base avec du contenu différent.
    Path(backup_mod.DB_PATH).write_bytes(b"AFTER_MODIFICATION")
    ...
    assert Path(backup_mod.DB_PATH).read_bytes() == b"FAKE_SQLITE_CONTENT_FOR_TEST"
